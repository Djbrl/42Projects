		...
		while (j < data->width)
		{
			if (data->map[i][j] == '1')
				mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, \
					data->wall, j * 1080 / data->width, i * 640 / data->height);
			if (data->map[i][j] == '0')
				mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, \
					data->path, j * 1080 / data->width, i * 640 / data->height);
			if (data->map[i][j] == 'C')
				mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, \
					data->item, j * 1080 / data->width, i * 640 / data->height);
			if (data->map[i][j] == 'P')
			{
				data->player_x = i;
				data->player_y = j;
				mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, \
				data->player, j * 1080 / data->width, i * 640 / data->height);
			}
			if (data->map[i][j] == 'E')
				mlx_put_image_to_window(data->mlx_ptr, data->win_ptr, \
					data->exit, j * 1080 / data->width, i * 640 / data->height);
			j++;
		}
		j = 0;
		i++;
...
int	main(int ac, char **av)
{
	...
	data.mlx_ptr, 1080, 640, "The Mighty Quest For The Epic Loot")))
		return (exit_error(MLX_WIN_ERR, &data));
	data.items_left = get_items(&data);
	data.score = 0;
	data.wall = create_image(&data, (640 - padding) / data.height, \
		(1080 - padding) / data.width, 0x444);
	data.player = create_image(&data, (640 - padding) / data.height, \
		(1080 - padding) / data.width, 0x880808);
	data.path = create_image(&data, (640 - padding) / data.height, \
		(1080 - padding) / data.width, 0x0);
	data.item = create_image(&data, (640 - padding) / data.height, \
		(1080 - padding) / data.width, 0x777);
	data.exit = create_image(&data, (640 - padding) / data.height, \
